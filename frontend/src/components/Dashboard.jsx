...
const STAT_CARDS = [
  {
    label: 'Total Gestures', key: 'total_gestures', modalKey: 'gestures',
    icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 11V6a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v0M14 10V4a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v2M10 10.5V6a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v8" /><path d="M18 8a2 2 0 1 1 4 0v6a8 8 0 0 1-8 8h-2c-2.8 0-4.5-.86-5.99-2.34l-3.6-3.6a2 2 0 0 1 2.83-2.82L7 15" /></svg>),
    accent: '#3b82f6', bg: 'rgba(59,130,246,0.08)', border: 'rgba(59,130,246,0.2)',
  },
  {
    label: 'Unique Students', key: 'unique_students', modalKey: 'students',
    icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>),
    accent: '#10b981', bg: 'rgba(16,185,129,0.08)', border: 'rgba(16,185,129,0.2)',
  },
  {
    label: 'Total Alerts', key: 'total_alerts', modalKey: 'alerts',
  ...
  {
    label: 'Avg Response', key: null, modalKey: null, value: 'N/A',
...
function StatModal({ modalKey, stats, onClose }) {
  ...
  const titles = { gestures: 'Total Gestures Breakdown', students: 'Unique Students Detail', alerts: 'Total Alerts Detail' }
        ...
        <div>
          {perStudent.length === 0
            ...
            : perStudent.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#10b981' }}>{i + 1}</div>
                  <div>
        ...
        <div>
          {alertLogs.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '24px 0' }}>
              <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="rgba(148,163,184,0.4)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ display: 'block', margin: '0 auto 10px' }}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '10px 0 10px 12px', borderBottom: '1px solid #f1f5f9', borderLeft: '3px solid rgba(239,68,68,0.45)', marginBottom: 2 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: '#94a3b8', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {a.date && <span>{a.date}</span>}{a.time && <span>· {a.time}</span>}
                  {a.chair_rank && <span>· Chair #{a.chair_rank}</span>}{a.student_name && <span>· {a.student_name}</span>}
                </div>
              </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }} onClick={onClose}>
      <div style={{ background: 'white', borderRadius: 18, boxShadow: '0 24px 64px rgba(0,0,0,0.16)', width: 480, maxWidth: '90vw', maxHeight: '80vh', display: 'flex', flexDirection: 'column', border: '1px solid rgba(0,0,0,0.06)' }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          ...
          <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: '50%', background: '#f1f5f9', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#64748b' }}>×</button>
        </div>
        <div style={{ padding: '4px 24px 24px', overflowY: 'auto', flex: 1 }}>{renderContent()}</div>
      </div>
    </div>
  )
}
...
function SaveSuccessModal() {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10001 }}>
      <div style={{ background: 'white', borderRadius: 20, boxShadow: '0 24px 64px rgba(0,0,0,0.18)', width: 400, maxWidth: '90vw', padding: '36px 32px', textAlign: 'center', border: '1px solid rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
          {[0, 0.2, 0.4].map((delay, i) => (
            <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: '#10b981', animation: `cs-pulse 1s ease-in-out ${delay}s infinite` }} />
          ))}
        </div>
      </div>
    </div>
...
function NotificationPanel({ alerts, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', top: 70, right: 20, width: 340, background: 'white', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.16)', border: '1px solid rgba(0,0,0,0.06)', zIndex: 9000, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
      <div style={{ padding: '16px 18px 12px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>{unread} unread alert{unread !== 1 ? 's' : ''}</div>
        </div>
        <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: '50%', background: '#f1f5f9', border: 'none', cursor: 'pointer', fontSize: 16, color: '#64748b', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
      </div>
      <div style={{ maxHeight: 380, overflowY: 'auto' }}>
        {alerts.length === 0 ? (
          <div style={{ padding: '32px 20px', textAlign: 'center' }}>
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgba(148,163,184,0.4)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ display: 'block', margin: '0 auto 10px' }}>
              <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/>
            </svg>
            <div style={{ fontSize: 13, color: '#94a3b8', fontWeight: 500 }}>No alerts yet</div>
            <div style={{ fontSize: 11, color: '#cbd5e1', marginTop: 4 }}>Alerts appear during Exam or Quiz sessions</div>
          </div>
        ) : alerts.map((alert, i) => (
          <div key={i} style={{ padding: '12px 18px', borderBottom: '1px solid #f8fafc', display: 'flex', gap: 12, alignItems: 'flex-start', background: alert.read ? 'white' : 'rgba(239,68,68,0.02)' }}>
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: alert.type === 'alert' ? 'rgba(239,68,68,0.1)' : 'rgba(245,158,11,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={alert.type === 'alert' ? '#ef4444' : '#f59e0b'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/>
                <line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>
              </svg>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#0f172a', textTransform: 'capitalize', marginBottom: 2 }}>
                {(alert.gesture || 'Unknown').replace(/_/g, ' ')} detected
              </div>
              <div style={{ fontSize: 11, color: '#94a3b8', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {alert.chair_rank && <span>Chair #{alert.chair_rank}</span>}
                {alert.student_name && <span>· {alert.student_name}</span>}
                {alert.time && <span>· {alert.time}</span>}
              </div>
            </div>
            {!alert.read && <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#ef4444', flexShrink: 0, marginTop: 4 }} />}
          </div>
        ))}
      </div>
    </div>
...
export default function Dashboard({ onLogout, setupPrefs, updateSetupPrefs }) {
  ...
  useEffect(() => {
    const tick = () => {
      const now = new Date()
      setLiveDateTime(`${now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} | ${now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}`)
    }
    tick()
  ...
  const fetchStats = useCallback(async (date, mode) => {
    try {
      const url = mode === 'total'
        ...
        : `${API_BASE}/dashboard/stats/?date=${toLocalDateStr(date)}`
      const res = await fetch(url)
      const data = await res.json()
      setStats(data)
      if (data.recent_alerts?.length) {
        setNotifications(prev => {
          const existingKeys = new Set(prev.map(n => `${n.time}${n.gesture}`))
          const newOnes = data.recent_alerts
            .filter(a => !existingKeys.has(`${a.time}${a.gesture}`))
            .map(a => ({ ...a, type: 'alert', read: false }))
  ...
  useEffect(() => { fetchStats(selectedDate, viewMode) }, [selectedDate, viewMode, fetchStats])

  useEffect(() => {
    if (active === 'Dashboard') {
      autoRefreshRef.current = setInterval(() => fetchStats(selectedDate, viewMode), 30000)
  ...
  useEffect(() => {
    if (active === 'Dashboard') fetchStats(selectedDate, viewMode)
  }, [active])
  ...
  const handleDateSelect = (newDate) => { setSelectedDate(newDate); setViewMode('day'); setShowCalendar(false) }
  ...
  const handleSaveSuccess = () => {
    ...
    setTimeout(() => {
      setShowSaveSuccess(false); markClean(); setSession(null); setActive('Dashboard')
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes cs-bell { 0%,100%{transform:rotate(0)} 20%{transform:rotate(-14deg)} 40%{transform:rotate(14deg)} 60%{transform:rotate(-8deg)} 80%{transform:rotate(8deg)} }
        .cs-app-root { display:flex; height:100vh; background:#f1f5f9; font-family:'DM Sans','Segoe UI',sans-serif; overflow:hidden; }
        ...
        .cs-notif-btn {
          ...
          cursor:pointer; transition:all 0.18s ease; color:rgba(186,210,235,0.8);
        }
        .cs-notif-btn:hover { background:rgba(59,130,246,0.15); border-color:rgba(59,130,246,0.3); color:#e0f2fe; }
        .cs-notif-btn.has-unread svg { animation:cs-bell 0.8s ease 0s 1; }
        .cs-notif-dot {
          position:absolute; top:4px; right:4px;
          min-width:16px; height:16px; border-radius:8px;
          background:#ef4444; border:2px solid rgba(15,23,42,0.97);
          display:flex; align-items:center; justify-content:center;
          font-size:9px; font-weight:700; color:white; line-height:1; padding:0 3px;
        }
        .cs-avatar-btn {
          width:34px; height:34px;
          background:linear-gradient(135deg,#3b82f6,#1d4ed8);
          border-radius:50%; display:flex; align-items:center; justify-content:center;
          cursor:pointer; box-shadow:0 2px 8px rgba(59,130,246,0.3); transition:all 0.18s ease;
        }
        .cs-avatar-btn:hover { transform:scale(1.05); }
        ...
        .cs-view-toggle { display:flex; align-items:center; gap:6px; margin-left:auto; }
        .cs-toggle-btn {
          padding:4px 12px; border-radius:6px; font-size:11px; font-weight:700;
          cursor:pointer; border:1px solid transparent;
          font-family:'DM Sans','Segoe UI',sans-serif; transition:all 0.18s ease; letter-spacing:0.3px;
        }
        .cs-toggle-btn.active { background:#3b82f6; color:white; border-color:#3b82f6; box-shadow:0 2px 8px rgba(59,130,246,0.3); }
        .cs-toggle-btn.inactive { background:rgba(59,130,246,0.08); color:#3b82f6; border-color:rgba(59,130,246,0.2); }
        ...
        .cs-stat-card:hover { transform:translateY(-3px); background:rgba(255,255,255,0.84); box-shadow:0 12px 32px rgba(0,0,0,0.12); }
        .cs-stat-card.no-click { cursor:default; }
        .cs-click-hint { position:absolute; top:9px; right:11px; font-size:9px; color:rgba(148,163,184,0.5); font-weight:600; letter-spacing:0.3px; text-transform:uppercase; }
        ...
        .cs-total-badge { display:inline-flex; align-items:center; gap:4px; background:linear-gradient(135deg,rgba(168,85,247,0.12),rgba(59,130,246,0.12)); border:1px solid rgba(168,85,247,0.25); border-radius:6px; padding:2px 8px; font-size:10px; font-weight:700; color:#a855f7; margin-left:8px; vertical-align:middle; }
      `}</style>

      <div className="cs-app-root">
        <Sidebar active={active} setActive={handleNavigation} onLogout={onLogout} setupPrefs={setupPrefs} />
        ...
        <div className="cs-main-area">
          {active !== 'Start Class' && (
            <header className="cs-topbar">
              ...
              <div className="cs-topbar-right">
                {/* Notification Bell */}
                <button
                  className={`cs-notif-btn ${unreadCount > 0 ? 'has-unread' : ''}`}
                  onClick={e => { e.stopPropagation(); setShowNotifications(v => !v); setNotifications(prev => prev.map(n => ({ ...n, read: true }))) }}
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/>
                  </svg>
                  {unreadCount > 0 && <span className="cs-notif-dot">{unreadCount > 9 ? '9+' : unreadCount}</span>}
                </button>
                ...
                {/* Avatar — click goes to Profile Settings */}
                <div className="cs-avatar-btn" onClick={() => handleNavigation('Profile Settings')}>
          ...
          {showNotifications && (
            <NotificationPanel alerts={notifications} onClose={() => setShowNotifications(false)} />
          )}
          ...
          {active === 'Dashboard' && (
            <main className="cs-dashboard-content">
              <div className="cs-date-bar">
                ...
                {viewMode === 'total' ? <>All Time Data <span className="cs-total-badge">★ TOTAL</span></> : <>{isToday ? 'Today — ' : ''}{dateLabel}</>}
                <div className="cs-view-toggle">
                  <button className={`cs-toggle-btn ${viewMode === 'day' ? 'active' : 'inactive'}`} onClick={() => setViewMode('day')}>
                    {isToday ? 'Today' : 'Selected Day'}
                  </button>
                  <button className={`cs-toggle-btn ${viewMode === 'total' ? 'active' : 'inactive'}`} onClick={() => setViewMode('total')}>
                    All Time Total
                  </button>
                </div>
              </div>
              ...
              <div className="cs-stat-grid">
                {STAT_CARDS.map(({ label, key, value, icon, accent, bg, border, modalKey }) => (
                  <div key={label} className={`cs-stat-card${!modalKey ? ' no-click' : ''}`} onClick={() => modalKey && setActiveModal(modalKey)}>
          ...
          {active === 'Start Class' && (
            <StartClass selectedDate={selectedDate} onDateSelect={handleDateSelect} setupPrefs={setupPrefs} updateSetupPrefs={updateSetupPrefs} onSaveSuccess={handleSaveSuccess} />
          )}
          {active === 'Class Setup' && <ClassSetup setupPrefs={setupPrefs} updateSetupPrefs={updateSetupPrefs} />}
          {active === 'Profile Settings' && <ProfileSettings setupPrefs={setupPrefs} updateSetupPrefs={updateSetupPrefs} onLogout={onLogout} />}
