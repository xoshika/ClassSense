...
from .models import Alert, ClassSession, GestureLog, StudentRoster, UserPreference
...
def gesture_logs(request):
    ...
    return Response({**GestureLogSerializer(log).data, "alert": alert_data}, status=201)
...
@api_view(["GET"])
def class_activity_logs(request):
    qs = GestureLog.objects.all()
    date_str = request.query_params.get("date")
    session_id = request.query_params.get("session_id")
    if date_str:
...
@api_view(["GET", "POST"])
def class_setup_preferences(request):
    if request.method == "GET":
        pref = UserPreference.objects.filter(user__isnull=True).first()
        if pref:
            return Response({
                "num_chairs": pref.num_chairs,
                "subject_name": pref.subject_name,
                "teacher_name": pref.teacher_name,
                "room_number": pref.room_number,
            })
        return Response({"num_chairs": 12, "subject_name": "", "teacher_name": "", "room_number": ""})
    if request.method == "POST":
        num_chairs = request.data.get("num_chairs", 12)
        subject_name = request.data.get("subject_name", "")
        teacher_name = request.data.get("teacher_name", "")
        room_number = request.data.get("room_number", "")
        pref, _ = UserPreference.objects.get_or_create(user=None)
        pref.num_chairs = num_chairs
        pref.subject_name = subject_name
        pref.teacher_name = teacher_name
        pref.room_number = room_number
        pref.save()
